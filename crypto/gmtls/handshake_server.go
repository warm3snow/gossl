...
func (hs *serverHandshakeState) processClientHello() error {
	...
	if priv, ok := hs.cert.PrivateKey.(gocrypto.Signer); ok {
	...
	if priv, ok := hs.cert.PrivateKey.(gocrypto.Decrypter); ok {
		switch priv.Public().(type) {
		case *rsa.PublicKey:
...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	var pub gocrypto.PublicKey // public key for client auth, if any
	...
	preMasterSecret, err := keyAgreement.processClientKeyExchange(c.config, hs.cert, pub, ckx, c.vers, false)
	if err != nil {
		c.sendAlert(alertHandshakeFailure)
		return err
	}
	...
	if len(c.peerCertificates) > 0 {
		...
		if err := verifyHandshakeSignature(sigType, pub, gocrypto.Hash(sigHash), signed, certVerify.signature); err != nil {
