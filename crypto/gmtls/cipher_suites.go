...
type keyAgreement interface {
	...
	processClientKeyExchange(*Config, *Certificate, gocrypto.PublicKey, *clientKeyExchangeMsg, uint16, bool) ([]byte, error)

	// On the client side, the next two methods are called in order.

	// This method may not be called if the server doesn't send a
	// ServerKeyExchange message.
	processServerKeyExchange(*Config, *clientHelloMsg, *serverHelloMsg, *x509.Certificate, *x509.Certificate, *serverKeyExchangeMsg, bool) error
...
var cipherSuitesTLS13 = []*cipherSuiteTLS13{
	...
	// RFC 8998
	{TLS_SM4_GCM_SM3, 16, aeadSM4GCMTLS13, crypto.SM3},
	{TLS_SM4_CCM_SM3, 16, aeadSM4CCMTLS13, crypto.SM3},
}
...
func aeadSM4GCMTLS13(key []byte, nonceMask []byte) cipher.AEAD {
	if len(nonceMask) != aeadNonceLength {
...
func aeadSM4CCMTLS13(key []byte, nonceMask []byte) cipher.AEAD {
	if len(nonceMask) != aeadNonceLength {
		panic("tls: internal error: wrong implicit nonce length")
	}
	block, err := sm4.NewCipher(key)
	...
	aead, err := ccm.NewCCM(block, ccmTagSize, ccmStandardNonceSize)
	if err != nil {
		panic(err)
	}
