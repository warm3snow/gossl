...
func (c *Conn) makeClientHello() (*clientHelloMsg, ecdheParameters, error) {
	...
	for _, suiteId := range possibleCipherSuites {
		...
		// config can set TLS1.3 ciphersuites in tls.config
	...
	if hello.supportedVersions[0] == VersionTLS13 {
		// If config does not specify tls1.3 ciphersuites, then use default
		// If client set certificate in config, determine suite priority based on certificates
		if c.config.CipherSuites == nil {
			if len(c.config.Certificates) > 0 {
				switch c.config.Certificates[0].PrivateKey.(type) {
				case *sm2.PrivateKey:
					hello.cipherSuites = append(hello.cipherSuites, priorityGMCipherSuitesTLS13()...)
				default:
...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	if ok {
		...
		err = keyAgreement.processServerKeyExchange(c.config, hs.hello, hs.serverHello, c.peerCertificates[0], c.peerCertificates[0], skx, true)
		if err != nil {
			c.sendAlert(alertUnexpectedMessage)
			return err
		}
